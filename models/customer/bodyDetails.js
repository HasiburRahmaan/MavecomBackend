...
function validationBodyDetails(bodyDetails){
    const schema = {
        ...
        height: Joi.number().max(255).min(1).required(),
        weight: Joi.number().max(255).min(1).required(),
        neck:  Joi.number().max(255).min(1),
        thigh: Joi.number().max(255).min(1),
        chest: Joi.number().max(255).min(1),
        belly: Joi.number().max(255).min(1),
        color: Joi.string().max(255).min(1)
    }
