...
const {
  TopPreferableSeller,
  validationTopPreferableSeller
} = require("../../models/customer/topPreferableSeller");
...
exports.deleteTopPreferableSeller = async (req, res) => {
  const topPreferableSeller = await TopPreferableSeller.findById(
...
exports.updateTopPreferableSeller = async (req, res) => {
  ...
  const result = await TopPreferableSeller.findByIdAndUpdate(
    req.params.customerId,
    {
      $set: req.body
    }
