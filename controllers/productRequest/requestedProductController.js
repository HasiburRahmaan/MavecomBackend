...
const {RequestedProduct ,validateRequestedProduct}=require("../../models/ProductRequest/requestedProduct")
...
exports.getRequestedProductsById= async(request,response)=>{
    try{
        const requestedProduct=await RequestedProduct.findById(request.params.productId);
...
exports.putRequestedProductsById=async(request,response)=>{
    try{
       const requestedProduct=await RequestedProduct.findByIdAndUpdate(request.params.productId,{
           $set:request.body
       });
       return response.status(200).send(requestedProduct)
    }catch(err){
