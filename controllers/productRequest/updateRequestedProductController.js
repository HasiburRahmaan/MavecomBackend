...
const{validateUpdateRequestedProduct ,UpdateRequestedProduct}=require("../../models/ProductRequest/updateRequestedProduct")
...
exports.getUpdateRequestedProductById= async(request,response)=>{
    try{
        const updateRequestedProduct=await UpdateRequestedProduct.findById(request.params.productId);
...
exports.putUpdateRequestedProductById=async(request,response)=>{
    try{
       const updateRequestedProduct=await UpdateRequestedProduct.findByIdAndUpdate(request.params.productId,{
           $set:request.body
       });
       return response.status(200).send(updateRequestedProduct)
    }catch(err){
